...
from table_bert.vertical.dataset import collate,collate_col
import time
...
class VerticalAttentionTableBert(VanillaTableBert):
    ...
    def forward(
        ...
        **kwargs
        ...
        #print('hereeee')
        #print(flattened_input_ids.get_device())
        #print(flattened_segment_ids.get_device())
        #print(flattened_sequence_mask.get_device())
        ...
        #print(flattened_input_ids.shape)

        use_batches=False
        if use_batches:
            len_data=flattened_input_ids.shape[0]
            batch_size_bert=1
            num_batches = int(math.ceil(len_data / batch_size_bert))
            bert_output=[]

            for chunk_id in range(0, num_batches):
                flattened_input_ids_b = flattened_input_ids[batch_size_bert * chunk_id: batch_size_bert * chunk_id + batch_size_bert]
                flattened_segment_ids_b = flattened_segment_ids[batch_size_bert * chunk_id: batch_size_bert * chunk_id + batch_size_bert]
                flattened_sequence_mask_b = flattened_sequence_mask[batch_size_bert * chunk_id: batch_size_bert * chunk_id + batch_size_bert]
                #print(flattened_input_ids_b.shape)
                bert_output_b, _ = self.bert(
                    input_ids=flattened_input_ids_b,
                    token_type_ids=flattened_segment_ids_b,
                    attention_mask=flattened_sequence_mask_b,
                    **kwargs
                )
                #bert_output_b=bert_output_b.to('cpu')
                bert_output.append(bert_output_b)
                del bert_output_b
        ...
        # (batch_size, max_row_num, sequence_len, hidden_size)
        #bert_output=bert_output.to('cuda:0')
        bert_output = bert_output.view(batch_size, max_row_num, sequence_len, -1)
        #column_token_position_to_column_ids=column_token_position_to_column_ids.to('cuda:0')
        #context_token_positions=context_token_positions.to('cuda:0')
        #context_token_mask = context_token_mask.to('cuda:0')
        #table_mask = table_mask.to('cuda:0')
        ...
        #print(context_encoding.get_device())
        #torch.cuda.set_device(0)
        #print(context_encoding)
        #print(table_encoding)


        #context_encoding,context_token_mask,table_encoding,table_mask = context_encoding.to('cpu'),context_token_mask.to('cpu'),table_encoding.to('cpu'),table_mask.to('cpu')
        #context_encoding=context_encoding.to('cuda:0')
        #context_token_mask = context_token_mask.to('cuda:0')
        #table_encoding = table_encoding.to('cuda:0')
        #table_mask = table_mask.to('cuda:0')
        ...
        ##mean instead of vertical attention
        # (batch_size, 1, 1)
        table_row_nums = table_mask[:, :, 0].sum(dim=-1)[:, None, None]
        # (batch_size, context_len, hidden_size)
        context_encoding = context_encoding.sum(dim=1) / table_row_nums
        # (batch_size, max_column_num, hidden_size)
        schema_encoding = table_encoding.sum(dim=1) / table_row_nums
    ...
    def forward_row(
        ...
        **kwargs
        ...
        #context_encoding = context_encoding.to('cpu')
        #context_token_mask = context_token_mask.to('cpu')
        #table_encoding = table_encoding.to('cpu')
        #table_mask = table_mask.to('cpu')

        # perform vertical attention
        #context_encoding, schema_encoding, final_table_encoding = self.row_vertical_transform(
        #    context_encoding, context_token_mask, table_encoding, table_mask)
    ...
    def row_vertical_transform(self, context_encoding, context_token_mask, table_encoding, table_mask):
        ...
        hidden_states = bert_output
        vertical_layer_outputs = []
        for vertical_layer in self.row_vertical_transformer_layers:
            hidden_states = vertical_layer(hidden_states, attention_mask=attention_mask)
            vertical_layer_outputs.append(hidden_states)
    ...
    def to_tensor_dict(
        ...
        for e_id, (context, table) in enumerate(zip(contexts, tables)):
            ...
            for col_inst in instance['cols']:
                col_inst['token_ids'] = self.tokenizer.convert_tokens_to_ids(col_inst['tokens'])
        ...
        tensor_dict_col = collate_col(examples, config=self.config, train=False)

        return tensor_dict, examples,tensor_dict_col
    ...
    def forward_query(
        ...
        **kwargs
        ...
        context_encoding=context_encoding.squeeze(1)
    ...
    def encode(
    ...
    ) -> Tuple[torch.Tensor, torch.Tensor, Dict,torch.Tensor,torch.Tensor,Dict]:
        assert return_bert_encoding is False, 'VerticalTableBert does not support `return_bert_encoding=True`'

        #print(tables)

        tensor_dict, instances,tensor_dict_col = self.to_tensor_dict(contexts, tables)
        ...
        #cuda_0_elements=['input_ids','segment_ids','sequence_mask']
        #tensor_dict = {
        #        k: v.to('cuda:0') if torch.is_tensor(v) and k in cuda_0_elements else v.to('cuda:1')
        #    for k, v in tensor_dict.items()
        #}
        ...
        tensor_dict_col = {
                k: v.to(self.device) if torch.is_tensor(v) else v
            for k, v in tensor_dict_col.items()
        }
        ...
        context_encoding_row, row_encoding = self.forward_row(**tensor_dict_col)

        tensor_dict_col['context_token_mask'] = tensor_dict_col['context_token_mask'][:, 0, :]
        tensor_dict_col['column_mask'] = tensor_dict_col['table_mask'][:, 0, :]

        info_row = {
            'tensor_dict': tensor_dict_col,

        }



        return context_encoding, schema_encoding, info,context_encoding_row,row_encoding,info_row
    ...
    def encode_query(
        ...
        # tensor_dict_col = {
        #     k: v.to('cpu') if torch.is_tensor(v) and k in cuda_0_elements else v
        #     for k, v in tensor_dict_col.items()
        # }

        context_encoding = self.forward_query(**tensor_dict)
