...
kfolds = np.load('pmc_folds.npy', allow_pickle=True)
kfolds = kfolds[()]
pmc_data = np.load('pmc_data.npy', allow_pickle=True)
pmc_data = pmc_data[()]
...
for fold_index,(train, test) in enumerate(kfolds):
    ...
    save_path = 'pmc_tabert_'+str(fold_index)+'.pt'
    save_path_best = 'best_pmc_tabert_'+str(fold_index)+'.pt'
